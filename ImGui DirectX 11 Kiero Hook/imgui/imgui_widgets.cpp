...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), IM_COL32_BLACK, dir, 1.1f);
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Text)), dir);
...
bool ImGui::ArrowButton(const char* str_id, ImGuiDir dir)
{
    ...
    return ArrowButtonEx(str_id, dir, ImVec2(sz, sz), ImGuiButtonFlags_AlignTextBaseLine);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        ...
        const float pad = ImMax(1.0f, IM_FLOOR(16 / 8.0f));

        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, IM_COL32(255, 123, 114, 255), style.FrameRounding);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 2, pad + 1), check_col, 16 - pad * 1.5f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 17));
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, frame_col, style.FrameRounding, ImDrawFlags_RoundCornersRight);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            //RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y - 1.f), IM_COL32(255, 255, 255, 255), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 1.2f);
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), IM_COL32(200, 200, 200, hovered ? 200 : 150), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 1.0f);
    }

    // Render preview and label
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, hovered ? 0.8f : 0.6f));
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderText(bb.Min + style.ItemInnerSpacing - ImVec2(0.f, 3.f), preview_value, NULL, NULL);
        ImGui::PopStyleColor();
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y - 6.f)); // Horizontally align ourselves with the framed text
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 11));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), grab_bb.Max + ImVec2(2, 1), IM_COL32(0x5c, 0x79, 0x94, 255), style.FrameRounding);
        //window->DrawList->AddCircleFilled(grab_bb.GetCenter() + ImVec2(5, 0), 4.f, IM_COL32(255, 255, 255, 200));
        window->DrawList->AddLine(ImVec2(grab_bb.Max.x + 2, frame_bb.Min.y + 1), ImVec2(grab_bb.Max.x + 2, frame_bb.Max.y - 1), IM_COL32(255, 255, 255, 200));

        if (hovered || value_changed)
        {

            const auto value_size = ImGui::CalcTextSize(value_buf);
            const ImRect popup_bb((grab_bb.Min.x - 6) - (value_size.x / 2), grab_bb.Min.y + 14, (grab_bb.Max.x + 6) + (value_size.x / 2), grab_bb.Max.y + 22);

            ImGui::GetForegroundDrawList()->AddRectFilled(popup_bb.Min, popup_bb.Max, ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_PopupBg)), style.PopupRounding);
            ImGui::GetForegroundDrawList()->AddRect(popup_bb.Min, popup_bb.Max, ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border)), style.PopupRounding);
            ImGui::GetForegroundDrawList()->AddText(popup_bb.GetCenter() - value_size * 0.5f, ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Text)), value_buf, value_buf_end);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (!selected) window->DrawList->AddRect(bb.Min + ImVec2(6, 0), bb.Min + ImVec2(6 + 12.f, 12.f), bdr_col, 0.f);
    ...
    window->DrawList->AddRectFilled(bb.Min + ImVec2(7, 1), bb.Min + ImVec2(6 + 11.f, 11.f), bg_col, 0.f);
    ...
    RenderTextClipped(bb.Min + ImVec2(24, -2), bb.Min + ImVec2(6 + 12.f, 6.f), label, NULL, &label_size, style.SelectableTextAlign, &bb);
